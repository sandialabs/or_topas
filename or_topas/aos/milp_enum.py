...
from or_topas.aos import gurobi_generate_solutions, enumerate_binary_solutions, enumerate_linear_solutions
...
def enumerate_mixed_integer_linear_program_solutions(
    ...
    """
    supported_integer_program_methods = {'gurobi_generate_solutions' : gurobi_generate_solutions, 
                                         'enumerate_binary_solutions' : enumerate_binary_solutions}
    supported_linear_program_methods = {'enumerate_linear_solutions' : enumerate_linear_solutions}

    assert integer_program_method in supported_integer_program_methods, f"Requested {integer_program_method=} not in {supported_integer_program_methods=}"
    assert linear_program_method in supported_linear_program_methods, f"Requested {linear_program_method=} not in {supported_linear_program_methods=}"

    #pool managers set up
    ip_pm_name = f"{model.name}_aos_ip_pool"
    milp_pm_name = f"aos_milp_pool_index_"
    ip_pool_manager = PyomoPoolManager(name=ip_pm_name, policy=PoolPolicy.keep_best, as_solution=custom_as_solution)
    milp_first_pool_name = milp_pm_name + '0'
    milp_pool_manager = PyomoPoolManager(name=milp_first_pool_name, policy=PoolPolicy.keep_best, as_solution=custom_as_solution)
    ...
    for index, solution in enumerate(ip_pool_manager.solutions):
        pyomo_utils.load_solution_into_model(model, 
                             solution, 
                             descend_into = True,
        ...
        if not merge_all_results_pools:
            ...
            milp_pool_manager.add_pool(name=milp_current_pool_name, policy=PoolPolicy.keep_best, as_solution=custom_as_solution)
