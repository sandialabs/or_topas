...
class TestBendersSolver(unittest.TestCase):
    ...
    @unittest.skipIf(not gurobi_available, "gurobi is not available.")
    @unittest.skipIf(not numpy_available, "numpy is not available.")
    def test_bucketed_farmer_1(self):
        mip_solver = "gurobi"
        transform = "standard_lp"
        t0 = time.time()
        opt, m = tc.Farmer.setup_farmer(
    ...
    @unittest.skipIf(not gurobi_available, "gurobi is not available.")
    @unittest.skipIf(not numpy_available, "numpy is not available.")
    def test_bucketed_farmer_2(self):
        mip_solver = "gurobi"
        transform = "standard_lp"
        t0 = time.time()
        opt, m = tc.Farmer.setup_farmer(
        ...
        for i in range(10):
            self.assertAlmostEqual(
                m.buckets.land_use_indicators["CORN", i + 1].value,
                buckets_corn[i + 1],
                7,
            )
            self.assertAlmostEqual(
                m.buckets.land_use_indicators["SUGAR_BEETS", i + 1].value,
                buckets_sugar_beets[i + 1],
                7,
            )
            self.assertAlmostEqual(
                m.buckets.land_use_indicators["WHEAT", i + 1].value,
                buckets_wheat[i + 1],
                7,
            )
